...
void test_filesystem(){
	vfs_mkdir("/tmp");
	vfs_mkdir("/tmp/text");
	struct file *fd = vfs_open("/tmp/text/hi.txt", O_CREAT);
	char *str = "hihihihihoooo";
	vfs_write(fd, str, strlen(str));
	vfs_close(fd);

	struct file *fd2 = vfs_open("/tmp/text/hi.txt", 0);
	char buf[1024];
	vfs_read(fd2, buf, strlen(str));
	vfs_close(fd2);
...
void test_initramfs(){
	...
	struct file *fd = vfs_open("/initramfs/file1", 0);
	vfs_read(fd, buf, 1024);
	printf("name : %s\n", ((struct tmpfs_data *)(fd->vnode->internal))->name );
	printf("vfs read (file1) : %s\n", buf);
	vfs_close(fd);
}
...
void shell()
{
	...
	while(1)
	{
		...
		else if( strcmp(cmd, "file") == 0)		test_filesystem();
		else if( strcmp(cmd, "initramfs") == 0)	test_initramfs();
